    ...
    <section class="home-section h-screen mx-auto px-3 py-30">
        ...
        <div class="home-content">
            <div class="overview-boxes boxes-elt">
                <div class="container">
                    <div class="box1">
                        <form action="<?= isset($candidate) ? '?edit=' . $candidate['id_c'] : '' ?>" method="POST" enctype="multipart/form-data" style="width: 86%; margin-left: 5em;">
                                ...
                                <select name="id_e" id="id_e">
                                    ...
                                    <?php foreach ($etudiantes_niveau1 as $etudiante): ?>
                                        <option value="<?= $etudiante['id_e']; ?>"
                                            <?= (isset($candidate) && $candidate['id_e'] == $etudiante['id_e']) ? 'selected' : '' ?>>
                                            <?= $etudiante['nom_e']; ?> <?= $etudiante['prenom_e']; ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div><br><br>
                            ...
                            <div class="form-group">
                                ...
                                <?php if(isset($candidate) && !empty($candidate['photo'])): ?>
                                    <img src="<?= PATH ?>assets/img/<?= $candidate['photo'] ?>" width="80" style="margin-top:10px; border-radius:50%;">
                                <?php endif; ?>
                            </div><br><br>

                            <div class="form-group">
                                <input type="submit" value="<?= isset($candidate) ? 'Update candidate' : 'Add candidate' ?>" name="<?= isset($candidate) ? 'update' : 'add' ?>">
                            </div>
                        </form>
                    </div>
                                ...
                                <tr>
                                    <td>
                                        <?php if(!empty($cand['photo'])): ?>
                                            <img src="<?= PATH ?>assets/img/<?= $cand['photo'] ?>" width="50" height="50" style="border-radius:50%; object-fit:cover;">
                                        <?php else: ?>
                                    ...
                                    <td>
                                        <a href="<?= PATH ?>candidate?edit=<?= $cand['id_c'] ?>" class="btn btn-edit">Edit</a>
                                        <a href="<?= PATH ?>candidate?delete=<?= $cand['id_c'] ?>" class="btn btn-delete">Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
